...
with st.sidebar:
    st.markdown(":green[**Seleção de empresas e datas**]", help="Selecione as empresas e o intervalo\
    ...
    with st.expander("Clique para expandir"):
        tipo = st.radio("Escolha entre análise e previsão", ["Análise", "Previsão"])
        horizonte_previsao = st.number_input("Quantos dias gostaria de Prever?", min_value=1, max_value=15, value=6)
...
if processar and tipo=="Análise":
    ...
    with tab1:              
            st.subheader("Gráfico de Velas", divider="green")     
    ...
    with tab3:    
     ...
     with col2:
         ...
         novas_previsoes_df = pd.DataFrame({"Previsões Futuras": novas_previsoes.flatten()}, 
                                           index=pd.date_range("26-03-2025", periods=len(novas_previsoes), freq="B"))
